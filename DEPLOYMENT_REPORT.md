...
**Date:** 2026-02-21  
**Server:** mgc-saas-apps-1 (REDACTED_VPS_IP)  
...
### Connection Command
```bash
./remo connect --server remo@REDACTED_VPS_IP:22 --subdomain myapp --upstream http://127.0.0.1:3000 --tui
```
...
ssh -i /tmp/remo-test-key -N -R 0:localhost:3000 remo@REDACTED_VPS_IP
# Output: "Allocated port 33507 for remote forward to localhost:3000"
```

**What Doesn't Work:**
```bash
./remo connect --server remo@REDACTED_VPS_IP:22 --subdomain test --upstream http://127.0.0.1:3000
